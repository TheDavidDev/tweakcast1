...
const fs = require("fs");
const orders = require("./localDB/order.json");
...
app.get('/payment/:order_id', async (req, res) => {
    if (orders.length) {
        const paymentData = await orders.find(order => order.order_id === req.params.order_id)
...
app.post('/payment', (req, res) => {
    axios.post(`${KLARNA_API_ENDPOINT}/checkout/v3/orders`, req.body, {
        ...
        .then(function (response) {
            ...
            fs.writeFile("./localDB/order.json", JSON.stringify(orders), err => {
                if (err) throw err; 
